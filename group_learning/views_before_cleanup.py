    ...
    def _get_session_stats(self, session):
        ...
        try:
            ...
            if session.current_mission and total_teams > 0:
                try:
                    # Use the new session-wide completion tracking
                    session_completion_stats = SimplifiedPhaseInput.get_session_completion_stats(
                        session, session.current_mission
                    )
                    
                    completion_stats.update({
                        'completed_teams': session_completion_stats['completed_teams'],
                        'in_progress_teams': session_completion_stats['in_progress_teams'],
                        'completion_percentage': session_completion_stats['completion_percentage']
                    })

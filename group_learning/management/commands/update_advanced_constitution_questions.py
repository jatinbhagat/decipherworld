...
class Command(BaseCommand):
    help = 'Update Advanced Constitution Challenge with sophisticated questions based on basic game themes'

    def handle(self, *args, **options):
        self.stdout.write("🏛️ Updating Advanced Constitution Challenge Questions...")
        ...
        try:
            with transaction.atomic():
                ...
                if not game:
                    self.stdout.write(self.style.ERROR("❌ Advanced Constitution Game not found! Run create_advanced_constitution_game first."))
                    return
                
                # Clear existing questions
                existing_count = ConstitutionQuestion.objects.filter(game=game).count()
                if existing_count > 0:
                    self.stdout.write(f"🔄 Clearing {existing_count} existing questions...")
                    ConstitutionQuestion.objects.filter(game=game).delete()
                
                # Create sophisticated questions based on basic game themes
                questions_data = [
                    {
                        'order': 1,
                        'category': 'governance',
                ...
                for q_data in questions_data:
                    ...
                    self.stdout.write(f"✅ Created sophisticated question {q_data['order']}: {q_data['category']}")
                
                self.stdout.write(self.style.SUCCESS(f"🎉 Successfully created {len(questions_data)} sophisticated constitution questions!"))
                self.stdout.write(f"📊 Scoring system: Each question has one +3, one -3, one +1, and one -1 option")
                self.stdout.write(f"🧠 Focus: Constitutional principles, democracy, secularism with nuanced scenarios")
